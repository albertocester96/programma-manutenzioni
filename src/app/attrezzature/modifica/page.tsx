// src/app/attrezzature/modifica/[id]/page.tsx
'use client';
...
export default function EditEquipmentPage({ params }: Props) {
  ...
  const fetchEquipment = async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Errore durante il caricamento dell\'attrezzatura');
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Errore durante la modifica dell\'attrezzatura');
      }
      
      router.push(`/attrezzature/${params.id}`);
    } catch (error) {
      console.error('Errore nella modifica dell\'attrezzatura:', error);
      setErrors({
        submit: 'Si è verificato un errore durante la modifica dell\'attrezzatura'
      });
    } finally {
      setSubmitting(false);
    }
      ...
      <div className="bg-white shadow-md rounded-lg p-6">
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6 flex justify-end">
            <button
              type="button"
              onClick={() => router.push(`/attrezzature/${params.id}`)}
